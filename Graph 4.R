...
funcRevenueLeaks(dataset1,dataset2,dataset3,dataset4) # Function Call
...
# Main Function
funcRevenueLeaks   <- function(dataset1,dataset2,dataset3,dataset4){
  ...
  G4_Calc3_NULL = sqldf("select sum(SellingPrice) as SumSellingPrice, MarketPlace from selectedDataOnOrderDate where length(trim(DispatchDate))<3 group by MarketPlace")
  ...
  G4_Calc4_LateDelivery$lateDelivery
  ...
  df1 = rbind(df1, data.frame(xAxis="Cancellations",yAxis=sum(G4_Calc1_Cancelled$SellingPrice))) # Calc 1. OrderSales/Day
  df1 = rbind(df1, data.frame(xAxis="Returns",yAxis=sum(G4_Calc2_RTO$SellingPrice))) # Calc 2. Shipped Revenue / Day
  df1 = rbind(df1, data.frame(xAxis="Not Yet Delivered by the Date of data",yAxis=sum(G4_Calc3_NULL$SumSellingPrice))) # Calc 3.
  df1 = rbind(df1, data.frame(xAxis="Delivered later the Max Date",yAxis=G4_Calc4_LateDelivery$lateDelivery)) # Calc 4.
  df1 = rbind(df1, data.frame(xAxis="Market Place Fee",yAxis=G4_Calc5_MarketFee)) # Calc 5.
  
  df1 = rbind(df1, data.frame(xAxis="Payment Delay Revenue Loss",yAxis=G4_Calc6_PaymentDelayRevenueLoss)) #Calc 6. Per Day Realized Reveneue After Payment Delay
  ...
  ggplot(df1, aes(y=(yAxis/100000),x=xAxis)) +
    ...
    geom_text(aes(label = paste(round(yAxis/100000,2),"",sep=""),ymax=0),
              position=position_dodge(width=0.9), vjust=-0.25) + ylab("Lacs INR")+xlab("") +scale_y_continuous(labels = comma) + ggtitle("Revenue leaks") # Developed by : Prateek Singhal 
